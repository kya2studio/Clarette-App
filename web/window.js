...
// Kept in sync with the preset keys/labels in dockview-workspace.js (a separate
// window/JS context -- there's no shared module system to import this from).
const WORKSPACE_PRESET_LABELS=[['landscape','Landscape Mode'],['portrait','Portrait Mode']];
...
async function settings(){
 ...
 if(section==='Workspace & Preview'){
  ...
  const presetOptions=WORKSPACE_PRESET_LABELS.map(([id,label])=>[id,label]);
  const customOptions=Object.entries(ws.custom||{}).map(([id,w])=>[id,w.name]);
  html=`<section><label>Checker Brightness<span class="previewSettingControls"><input type="range" min="0" max="100" data-setting="checker_brightness" value="${state.settings.checker_brightness}"><button id="resetChecker" class="settingsReset" title="Reset Checker Background" aria-label="Reset Checker Background">${apiIcon('M3 4v6h6M3 10a9 9 0 1 1 1 8')}</button></span></label>${input('preview_background','Solid Preview Color','color')}</section><h2>Saved Batches</h2><div class="savedBatchRow"><select id="recoverBatch" aria-label="Saved Batches">${Object.values(state.batches).map(b=>`<option value="${esc(b.id)}" ${b.id===state.active?'selected':''}>${esc(b.name)}${b.closed_at?' · closed':''}</option>`).join('')}</select>${button('openBatch','Open Batch')}</div><h2>Workspace</h2><label>Layout<select id="workspaceList"><optgroup label="Presets">${presetOptions.map(([id,label])=>`<option value="${esc(id)}" ${id===ws.active?'selected':''}>${esc(label)}</option>`).join('')}</optgroup>${customOptions.length?`<optgroup label="Saved">${customOptions.map(([id,label])=>`<option value="${esc(id)}" ${id===ws.active?'selected':''}>${esc(label)}</option>`).join('')}</optgroup>`:''}</select></label><label>Drag Panels<input id="dragPanels" type="checkbox" ${ws.locked?'':'checked'}></label><p class="hint">Drag any panel by its tab to rearrange the workspace, drop on an edge to split, or drop in the center to combine panels into tabs. Save Workspace Layout, Reset Current Workspace and Restore Default Layout are in the Workspace menu.</p>`;
 }
 ...
 if($('workspaceList'))$('workspaceList').onchange=safe(async()=>{await api('/api/workspace',{operation:'select',id:$('workspaceList').value});await refresh();settings()});
...
async function dialog(){
 ...
 $('content').innerHTML=html+`<div class="buttons">${button('cancel','Cancel')}${button('submit',kind==='export'?'Export':kind==='new-batch'?'OK':'Save',true)}</div>`;$('cancel').onclick=close;
