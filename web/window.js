'use strict';
const $=id=>document.getElementById(id),query=new URLSearchParams(location.search),kind=query.get('kind')||'settings',sections=['General','Workspace & Preview','Output & Metadata','Cache & Performance','AI & APIs','Presets & Models','Toolbar & Apps','Keyboard Shortcuts'];
...
async function close(){await api('/api/close-window',{kind});window.close()}
...
async function settings(){
 ...
 if($('workspaceList')){$('workspaceList').onchange=safe(async()=>{await api('/api/workspace',{operation:'select',id:$('workspaceList').value});await refresh();settings()});const saveLayout=safe(async()=>{const ws={...state.workspaces[state.workspace],visible:{preview:true,...Object.fromEntries([...document.querySelectorAll('[data-panel-visible]')].map(el=>[el.dataset.panelVisible,el.checked]))}};await api('/api/workspace',{workspace:ws});state.workspaces[state.workspace]=ws;status('Workspace saved')});document.querySelectorAll('[data-panel-visible]').forEach(el=>el.onchange=saveLayout);if($('lockWorkspace'))$('lockWorkspace').onchange=safe(async()=>{await api('/api/workspace',{operation:'lock',locked:$('lockWorkspace').checked});await refresh();settings()})}
...
async function dialog(){
 ...
 if(kind==='workspace')html=`<label>Workspace Name<input id="name" value="${esc(query.get('operation')==='rename'?state.workspaces[state.workspace].name:'')}"></label><p class="hint">The current panel arrangement will be saved.</p>`;
 if(kind==='updates')html='<p>Clarette 1.0.1 · Build 1001</p><p>No update feed has been configured for this release. Install updates supplied by the developer.</p>';
 $('content').innerHTML=html+`<div class="buttons">${button('cancel','Cancel')}${kind==='updates'?'':button('submit',kind==='export'?'Export':kind==='new-batch'?'OK':'Save',true)}</div>`;$('cancel').onclick=close;
 if($('choose')){$('choose').onclick=safe(async()=>{const r=await api('/api/choose-path',{kind:'folder'});if(r.path){$('destination').value=r.path;resolve()}});const resolve=()=>{$('resolved').textContent=$('destination').value+'/'+$('name').value};$('name').oninput=$('destination').oninput=resolve;resolve()}
 if($('submit'))$('submit').onclick=safe(async()=>{
  ...
  if(kind==='workspace'){const op=query.get('operation');await api('/api/workspace',op==='rename'?{operation:'rename',name:$('name').value}:{operation:'create',workspace:{...state.workspaces[state.workspace],name:$('name').value}})}
  await close();
 });
}
...
function helpContent(){
 const workspaceSvg=`<svg viewBox="0 0 700 210" role="img" aria-label="Workspace layout: toolbar across the top, Portraits panel on the left, Preview in the center, Adjustments on the right" style="width:100%;height:auto">
  ...
  <text x="624" y="85" text-anchor="middle" fill="#9a95ad" font-size="10">Output, color, detail, mask</text>
 </svg>`;
...
<section><h2>Your workspace</h2>${workspaceSvg}<p class="hint">Portraits: batch and thumbnails. Preview: canvas, brush tools, comparisons, and where progress/notifications appear. Adjustments: output size, framing, color, detail and mask controls.</p></section>
...
<section><p class="hint">Clarette 1.0.1 · Build 1001</p></section>`;
}
async function start(){await refresh();if(kind==='settings')return settings();if(kind==='shortcuts'){section='Keyboard Shortcuts';return settings();}if(kind==='help'){$('title').textContent='Documentation & Tutorials';$('content').innerHTML=helpContent();return}return dialog()}
start().then(()=>{
 ...
 const outer=e=>{const s=getComputedStyle(e);return e.getBoundingClientRect().height+parseFloat(s.marginTop)+parseFloat(s.marginBottom)};
