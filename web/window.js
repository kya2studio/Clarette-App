...
async function settings(){
 ...
 if(section==='License'){
  const lic=state.licensed||{licensed:false,email:null,error:null},key=state.settings.license_key||'';
  html=lic.licensed
   ?`<section><h2>Clarette is activated</h2><p class="path">${esc(key.length>8?key.slice(0,4)+'••••••••'+key.slice(-4):key)}</p><div class="connection"><span>Status</span><span>Active</span></div><div class="connection"><span>Registered to</span><span>${esc(lic.email||'—')}</span></div>${button('deactivateLicense','Deactivate This Mac')}<p class="hint">Deactivating removes the license from this Mac. You'll need to enter a key again to keep using Clarette.</p></section>`
   :`<section><h2>Not activated</h2><p>${esc(lic.error||'Enter the license key from your purchase confirmation email.')}</p><label>License key<input id="settingsLicenseKey" type="text" autocomplete="off" spellcheck="false" value="${esc(key)}"></label>${button('activateLicenseSettings','Activate',true)}</section>`;
 }
